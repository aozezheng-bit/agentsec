...
- `reconcile-runtime`: reconcile an externally-produced, sanitized Runtime
  Attestation with a Pilot report directory; this never runs the target Agent.
...
The report directory also contains `homi-operationality.json`,
`homi-risk-state.json`, `homi-operation-context.json`,
`homi-context-risk.json`, `homi-risk-score.json`, `homi-posture.json` and
`homi-calibration.json`. They
are bound to
the exact Pilot JSON SHA-256 and classifies each static signal as
`template`, `latent`, `active`, or `runtime_attested`. The legacy
Operationality sidecar is signal-oriented; the RISK-02 Risk State sidecar also
classifies all six standard files and preserves an explicit `unknown` state for
missing/skipped or insufficiently classified coverage. The current static Homi
adapter can only emit the first three; `runtime_attested` remains unavailable
until an independent runtime attestation is supplied. These classifications are
not replacements for Severity or Evidence Confidence A/B/C/D.

`homi-operation-context.json` is the RISK-03 Pilot-bound structured extraction
of operation action, target, data classification, trigger, purpose,
authorization, controls, scope, frequency, and source Evidence. Unknown fields
remain unknown; no score, permission, or runtime authority is inferred.

`homi-context-risk.json` is the RISK-04 deterministic rule output over that
structured context. It looks for concrete operation combinations such as
sensitive data sent to an external destination, autonomous sensitive actions,
high-impact operations without authorization, Secret-to-external chains,
indefinite external persistence, and unauthorized control-file changes. A
public-web read of public data, persona/identity text, or long-term memory does
not trigger a high-risk Finding by itself. Unknown dimensions are emitted as
Coverage observations. RISK-04 is report-only and has no numeric score,
runtime proof, permission authority, authentication authority, or CI-blocking
authority. Numeric residual-risk quantification is exposed separately through
`homi-risk-score.json`.

`homi-risk-score.json` is the RISK-05 report-only quantification of the
Operation Context and RISK-04 report. It exposes four separate concepts:

- `potential_impact_score`: the highest NIST/AgentSec matrix representative;
- `residual_risk_score`: potential score adjusted only by explicit bounded
  control coverage, without averaging away a severe signal;
- `current_posture`: static evidence state; `current_posture_score` remains
  `null` without independent runtime attestation;
- `drift`: upward change relative to an explicitly supplied prior context/risk
  snapshot. Without a baseline, `drift_score` is `null`.

These scores are policy metrics for review and downstream service-side
decisions. They are not loss probabilities, runtime permissions, identity
attestations, Hard Gates, or CI blocking results.

To calculate baseline-relative drift, preserve a previous report directory and
pass it explicitly:

```bash
agentsec homi report /path/to/homi-workspace \
  --output-dir /tmp/agentsec-homi-current \
  --baseline-dir /tmp/agentsec-homi-baseline \
  --language zh --force
```

The baseline directory must contain the matching
`homi-operation-context.json` and `homi-context-risk.json`. Without an
explicit baseline, `drift_score` is `null`, not zero.

`homi-runtime-reconciliation.json` is the RISK-06 sidecar for external runtime
evidence. The host, not AgentSec, must produce the attestation from an approved
sandbox/telemetry service. The input is untrusted and value-minimized. The
reconciliation command verifies the Pilot Snapshot SHA-256, Operation Context
SHA-256, RISK-04 canonical report SHA-256, and the fixed report-only authority
flags. A verified, fully reconciled result may establish Evidence Confidence A
for the reconciled evidence only; it cannot grant permissions, authenticate an
Agent, modify Policy, block CI, or prove exploitability. Partial, conflicting,
unverified, undeclared, and Unknown observations remain explicit.
