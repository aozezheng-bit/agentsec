...
- Review Case: `review-case-d364bc35c0eb86e7071e`
- Synthetic: `true`
...
- `control` / `secret-handling` / `present` / `target:prodauto-2`
- `tool` / `mcp-enabled-required` / `absent` / `target:prodauto-2`
- `runtime_identity` / `identity-lifetime-reviewed` / `absent` / `target:prodauto-2`
- `relationship` / `memory` / `absent` / `target:prodauto-2`
