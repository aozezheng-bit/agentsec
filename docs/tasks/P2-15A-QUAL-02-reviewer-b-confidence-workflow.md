# P2-15A-QUAL-02：Reviewer B Confidence 校准工作流程
...
Reviewer B 只能访问项目负责人提供的 Confidence-only 工作目录，例如：
...
1. 不查看 Reviewer A 的目录或结果；
2. 不与 Reviewer A 讨论 Case 结论；
...
```text
我作为 Reviewer B 独立完成了 20 条 Confidence 校准，没有查看 Reviewer A、Ground Truth、Seed Label 或最终 Qualification 结果。本次只重新评审 Evidence Confidence，没有修改 Match/No-match 判断。
```
