...
## 13.4 P2-CAL-04 completion

P2-CAL-04 is complete for source development as of 2026-08-21:

```text
AdjudicationReviewSet Schema 0.1.0
Adjudication Report Output 0.2.0
122 seeded labels across 61 Case/Rule expectations
classification/category/disposition consensus and unresolved states
per-Rule TP/FP/FN/TN and tuning recommendation
three report-only Gate Candidate assessments
bilingual Text/JSON report
scripts/run-calibration-adjudication.py
adjudication JSON Schema exports
```

The Seed report has 61 consensus results and zero unresolved results. All three
Gate Candidates are `more_data_required` because the corpus has only one
positive and one negative/near-miss sample per component Rule and the labels are
seeded. P2-CAL-04 does not alter Rules, Risk Model, Hard Gates, CI, LLM, runtime
verification, or vulnerability proof. The next work is independent label
collection and corpus expansion before P2-15A.

## 13.5 P2-CAL-04A completion

P2-CAL-04A engineering preparation is complete as of 2026-08-24:

```text
expanded draft Corpus: 216 Cases, 431 Rule Expectations, Labels Version 0.2.0
blinded Reviewer Pack 0.3.0: 216 opaque Cases and 431 questions per Reviewer
ADR-0038 Reviewer/Adjudication provenance and seed/human evidence modes
Adjudication Resolution Set Schema 0.1.0
Adjudication Report Output 0.3.0
scripts/build-reviewer-pack.py build / validate / import CLI
scripts/check-gate-calibration-coverage.py report-only Coverage Check CLI
docs/calibration-adjudication-reviewer-pack.md human review guide
```

Draft Gate coverage now satisfies the volume targets with 25 unique Positive
and at least 21 unique eligible Negative/Near-miss scenarios per Gate, checked
by the Coverage CLI as `overall_status=ready`. This is sample volume readiness
only: every label remains `seeded`, all three Gate Candidates remain
`more_data_required`, and the required 20 reviewed Positive plus 20 reviewed
Negative/Near-miss samples per Gate do not exist yet. P2-CAL-04A does not
alter Rules, Risk Model, Hard Gates, CI, LLM, runtime verification, or
vulnerability proof, and it produces no Hard Gate qualification conclusion.
The next work is recruiting two real independent Reviewers and one Adjudicator,
collecting blind human labels, rerunning P2-CAL-04 in human evidence mode, and
only then evaluating P2-15A. `hard_gate=true`, CI blocking, and `--fail-on`
remain disabled.
...
The next independent work is a remote-source/cache policy if required, followed
by the already-planned CVSS report-only Gate and Phase 3 runtime verification.
