...
## Provider Pilot 所需的语义期望判断

为了让后续 Real Provider Evaluation 能计算 Precision / Recall，建议在每条 Review
Decision 中同时填写 `expected`。它表示你认为模型应该从该 Evidence 中识别出的语义
候选；它不是 AgentSec Finding，也不具有授权含义。
...
```json
{
  ...
  "evidence_ids": ["该 Case 的 evidence_id"]
}
```
...
不要为了“让模型通过”而增加候选。最终以专家判断为准；两位专家对 `expected` 有
分歧时，也需要在 Adjudication 中裁决。
