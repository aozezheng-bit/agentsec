...
def render_homi_capability_diff_html(
    ...
    *,
    ...
    substitutions = {
        ...
        "finding_label": "Finding 变化" if chinese else "Finding Delta",
        "capability_added_label": "新增" if chinese else "added",
        "capability_removed_label": "移除" if chinese else "removed",
        "capability_modified_label": "修改" if chinese else "modified",
        "finding_added_label": "新增" if chinese else "added",
        "finding_resolved_label": "已解决" if chinese else "resolved",
        "finding_changed_label": "已变化" if chinese else "changed",
        "signal_label": "能力信号" if chinese else "Signal",
        "change_label": "变化类型" if chinese else "Change",
        "before_label": "变更前" if chinese else "Before",
        "after_label": "变更后" if chinese else "After",
        "rule_id_label": "规则 ID" if chinese else "Rule ID",
        "delta_label": "变化" if chinese else "Delta",
        "before_severity_label": "变更前严重度" if chinese else "Before severity",
        "after_severity_label": "变更后严重度" if chinese else "After severity",
